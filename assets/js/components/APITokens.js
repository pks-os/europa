  ...
  renderIcons(token) {
    ...
    let infoIconDOM = (
      <i className='icon icon-dis-questionmark' key={1}
         onClick={() => {
         }} data-tip="A token must be INACTIVE to be deleted" data-for="ToolTipTop"/>
    );

    let deleteIconDOM = (
      <i className='icon icon-dis-trash' key={1}
         onClick={() => this.context.actions.toggleTokenForDelete(token.token)} data-tip="Delete Token"
         data-for="ToolTipTop"/>
    );

    let statusIconDOM = (
      <i className={statusIcon} key={2}
         onClick={() => this.setAuthTokenStatus(token.token, newStatus)} data-tip={statusToolTip}
         data-for="ToolTipTop"/>
    );
    ...
    return (
      <span className="Flex2 Token">
        ...
        <div className="ItalicText">
          ...
          <span className="ItalicItem"
                 onClick={() => this.context.actions.toggleShowingToken(tokenString)}>{verb}</span>&nbsp;&nbsp;
