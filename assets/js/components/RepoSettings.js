...
export default class RepoSettings extends Component {
  ...
  renderPublicSettings() {
    ...
    let publicAction = repo.publicRepo ? () => {
    } : () => this.context.actions.confirmPublicStatusChange(true)
    let privateAction = !repo.publicRepo ? () => {
    } : () => this.context.actions.confirmPublicStatusChange(false)
  ...
  renderPublicStatusChange() {
    ...
    if (NPECheck(this.props, 'repoDetails/publicConfirm', false)) {
      ...
      return (
        <CenteredConfirm
          message={`Are you sure you want make this repo ${(publicStatusChange) ? 'public' : 'private'}?`}
