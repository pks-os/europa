...
export default class DockerPullCommands extends Component {
  ...
  renderMore(selectedManifests) {
    if (selectedManifests && selectedManifests.length) {
      return (
        <div className="More" onClick={() => this.context.actions.toggleShowPullCommands()}>View selected tag pull
          commands...</div>
      );
    }
  }
              ...
              return (
                <div className="FlexColumn" key={index}>
                  <div className="LargePullCommand">
                    ...
                    <div className="NoClick" data-tip={manifest.manifestId}
                         data-for="ToolTipTop">{`${pullCommand}@${CleanSha(manifest.manifestId)}..`}</div>
